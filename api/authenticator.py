...
from fastapi import (
    Depends,
)  # Imports 'Depends' from FastAPI, a dependency injection system.
from jwtdown_fastapi.authentication import (
    Authenticator,
)  # Imports the 'Authenticator' class from 'jwtdown_fastapi.authentication' module.
from queries.accounts import (
    AccountRepo,
    AccountOutWithPassword,
)  # Imports several classes from the 'queries.accounts' module.


class ExampleAuthenticator(
    Authenticator
):  # Defines a new class 'ExampleAuthenticator' that inherits from 'Authenticator'.
    async def get_account_data(
        ...
        return accounts.get(
            email
        )  # Calls the 'get' method of 'AccountRepo' to fetch the account by email.
    ...
    def get_hashed_password(self, account: AccountOutWithPassword):
        ...
        return account.__getitem__(
            "hashed_password"
        )  # Gets the 'hashed_password' attribute from the 'account' and returns it.
    ...
    def get_account_data_for_cookie(self, account: AccountOutWithPassword):
        ...
        d = account[
            "user_name"
        ]  # Gets the 'user_name' attribute from the 'account' and assigns it to 'd'.
        return d, AccountOutWithPassword(
            **account
        )  # Returns two values: 'd' (user_name of the account) and an 'AccountOutWithPassword' object created from the account's dictionary representation.
