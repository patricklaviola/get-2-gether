...
from psycopg_pool import (
    ConnectionPool,
)  # Imports the 'ConnectionPool' class from the 'psycopg_pool' module.
...
)  # Initializes a connection pool using the DATABASE_URL from environment variables.


class Error(
    BaseModel
):  # Defines a 'Error' class which inherits from 'BaseModel'.
    message: str  # Defines a single attribute 'message' which should be of type 'str'.


class DuplicateAccountError(
    ValueError
):  # Defines a custom exception class 'DuplicateAccountError' which inherits from 'ValueError'.
    pass  # Placeholder indicating there's no additional code for this class.


class AccountIn(
    BaseModel
):  # Defines an 'AccountIn' class to represent incoming account data.
...
class AccountOut(
    BaseModel
):  # Defines an 'AccountOut' class to represent outgoing account data without password.
...
class AccountOutWithPassword(
    AccountOut
):  # Defines a class that extends 'AccountOut' but also includes the hashed password.
...
class AccountRepo:  # Defines a repository class 'AccountRepo' for account-related operations.
        ...
        try:  # Tries the following block of code.
            with pool.connection() as conn:  # Gets a connection from the connection pool.
                with conn.cursor() as db:  # Gets a database cursor.
                    ...
                    id = result.fetchone()[
                        0
                    ]  # Fetches the inserted user's ID.
    ...
    def get(
        ...
        AccountOutWithPassword
    ):  # Method to get a user from the database by their email.
        try:  # Tries the following block of code.
            with pool.connection() as conn:  # Gets a connection from the connection pool.
                with conn.cursor() as db:  # Gets a database cursor.
                    ...
                    result = db.execute(
                        ...
                        [
                            email
                        ],  # Uses the provided email for the where clause.
                    )
                    ...
                    return self.record_to_account_out(
                        record
                    )  # Converts the record to an 'AccountOutWithPassword' object and returns it.
        except Exception:  # Catches any exceptions.
            return {
